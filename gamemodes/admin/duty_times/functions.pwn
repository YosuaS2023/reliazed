...
ADuty_UpdateAllEndDuty()
{
  ...
  mysql_format(sqlcon, query, sizeof(query), "UPDATE `%s` SET `ended_at` = CURRENT_TIMESTAMP WHERE `ended_at` IS NULL;", ADMIN_DUTY_TIMES_TABLE_NAME);
  mysql_tquery(sqlcon, query, "OnADutyUpdateAllEndDuty");
  return 1;
}
...
AdminDutyTime_WriteStartTime(playerid)
{
  ...
  g_AdminDutyTimeLog[Account] = AccountData[playerid][uID];
...
AdminDutyTime_UpdateEndTime(playerid)
{
  ...
  new
    account_id = AccountData[playerid][uID],
    query[128],
    description[128],
    Timestamp:now = Now() + Hours:7,
    timestamp[64]
  ...
  mysql_format(sqlcon, query, sizeof(query), "UPDATE `%s` SET `ended_at` = CURRENT_TIMESTAMP WHERE `account` = %d AND `ended_at` IS NULL;", ADMIN_DUTY_TIMES_TABLE_NAME, account_id);
  mysql_tquery(sqlcon, query, "OnADutyEndDutyUpdated", "s", description);
  return 1;
}
