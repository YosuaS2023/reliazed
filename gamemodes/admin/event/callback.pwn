					...
					{ 
						if(IsPlayerInEvent(i))
						{
							if(eventTeams[i] == EventData[eventWinner]) 
							{
								...
								SendCustomMessage(i, "EVENT", "Team anda menang dalam event ini dan mendapat hadiah sebanyak "GREEN"%s.", FormatMoney(EventData[eventBonus]*3));
							}
							else
							{
								GiveMoney(i, EventData[eventBonus], ECONOMY_TAKE_SUPPLY, "Event prize");
								SendCustomMessage(i, "EVENT", "Kamu mendapat bonus "GREEN"%s "WHITE"dari partisipasi event.", FormatMoney(EventData[eventBonus]));
							} 
							eventLeave(i);
						}
					}
					...
					{ 
						if(IsPlayerInEvent(i))
						{
							if(i == EventData[eventWinner]){
								GiveMoney(i, EventData[eventBonus]*3, ECONOMY_TAKE_SUPPLY, "Event prize");
								SendCustomMessage(i, "EVENT", "Anda menang dalam event ini dan mendapat hadiah sebanyak "GREEN"%s.", FormatMoney(EventData[eventBonus]*3));
							}
							else
							{
								GiveMoney(i, EventData[eventBonus], ECONOMY_TAKE_SUPPLY, "Event prize");
		...
		if(eventCheckpoint[playerid] == (EventData[eventJetpackType]*16)) 
		{
		    ...
		    foreach(new i : Player) if(IsPlayerInEvent(i))
		    {
		    	if(EventData[eventWinner] == playerid)
					...
					{
						GiveMoney(playerid, EventData[eventBonus], ECONOMY_TAKE_SUPPLY, "Event prize");
						SendCustomMessage(playerid, "EVENT", "Kamu mendapat bonus {00FF00}%s "WHITE"dari partisipasi jetpack event.", FormatMoney(EventData[eventBonus]));
					}
