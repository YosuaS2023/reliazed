...
{
	mysql_pquery(sqlcon, "SELECT * FROM `speedcameras` ORDER BY `speedID` ASC LIMIT "#MAX_DYNAMIC_SPEED";", "Speed_Load", "");
}
...
Speed_Create(playerid, speed)
{
	...
	if((index = Iter_Free(Speed)) != cellmin)
	{
		...
		mysql_tquery(sqlcon, sprintf("INSERT INTO `speedcameras` (`speedLimit`) VALUES('%d');", speed), "OnSpeedCreated", "d", index);
		return index;
	}
	return -1;
}
...
Speed_Delete(index)
{
	if(Speed_IsExists(index))
	{
		...
		mysql_tquery(sqlcon, sprintf("DELETE FROM `speedcameras` WHERE `speedID`='%d';", SpeedData[index][speedID]));
...
Speed_Save(index, bool:save_speed = false)
{
	if(Speed_IsExists(index))
	{
		if(!save_speed)
		{
			mysql_tquery(sqlcon, sprintf("UPDATE speedcameras SET `speedX`='%.3f',`speedY`='%.3f',`speedZ`='%.3f',`speedAngle`='%.3f' WHERE `speedID`='%d';",
		...
		else mysql_tquery(sqlcon, sprintf("UPDATE `speedcameras` SET `speedLimit`='%d' WHERE `speedID`='%d';", SpeedData[index][speedMax], SpeedData[index][speedID]));
