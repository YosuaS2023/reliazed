...
Pump_Create(playerid, bizid)
{
    ...
    if((id = Iter_Free(GasPump)) != cellmin)
    {
        ...
        mysql_tquery(sqlcon, sprintf("INSERT INTO `pumps` (`ID`) VALUES(%d)", BusinessData[bizid][bizID]), "OnPumpCreated", "d", id);
        return id;
    }
    return -1;
}
...
Pump_Delete(pumpid)
{
    ...
    {
        mysql_tquery(sqlcon, sprintf("DELETE FROM `pumps` WHERE `pumpID` = '%d'", PumpData[pumpid][pumpID]));
...
Pump_Save(pumpid, save_all)
{
    if(Iter_Contains(GasPump, pumpid))
	{
		...
		else mysql_tquery(sqlcon, sprintf("UPDATE `pumps` SET pumpFuel=%d WHERE pumpID=%d;", PumpData[pumpid][pumpFuel], PumpData[pumpid][pumpID]));
	}
    return 1;
}
