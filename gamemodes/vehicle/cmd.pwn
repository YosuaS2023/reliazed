...
CMD:vm(playerid, params[])
	return callcmd::vehiclemenu(playerid, params);
	...
	switch(action)
	{
		...
		case 9: // Unmod
		{
			...
			if((vehicle_index = Vehicle_ReturnID(vehicleid)) != RETURN_INVALID_VEHICLE_ID)
			{
	            ...
	            // Modding remove
				mysql_tquery(sqlcon, sprintf("DELETE FROM `vehicle_mod` WHERE `vehicleid`='%d';", VehicleData[vehicle_index][vehIndex]));
		...
		case 11: // Plate Number
		{
			...
			if((vehicle_index = Vehicle_ReturnID(vehicleid)) != RETURN_INVALID_VEHICLE_ID)
			{
				...
				mysql_tquery(sqlcon, sprintf("UPDATE `server_vehicles` SET `plate`='%s' WHERE `id`='%d';", SQL_ReturnEscaped(VehicleData[vehicle_index][vehPlate]), VehicleData[vehicle_index][vehIndex]));
...
CMD:v(playerid, params[])
	return callcmd::vehicle(playerid, params);
...
CMD:scrapyard(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 10.0, 2410.4741, -1425.8549, 23.9830))
	{
		...
		if(Vehicle_IsOwned(playerid, vehicleid))
		{
			...
			format(string, sizeof(string), "Are you sure you want to destroy this vehicle for "GREEN"%s ?", FormatMoney(price));
...
Dialog:Scrapyard(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		SendServerMessage(playerid, "Kamu mendapatkan uang sejumlah "GREEN"%s "WHITE"dari kendaraanmu!", FormatMoney(price));
	...
	switch(action) 
	{
		case 1: // Engine
		{
			...
			if(GetEngineStatus(vehicleid))
			{
				...
				callcmd::ame(playerid, "reaches vehicle key, turn the ignition key to off.");
			}
			else 
			{
				...
				callcmd::ame(playerid, "reaches vehicle key, turn the ignition key to on.");
			}
		}
		case 2: // Lock
		{
		...
		case 7: //List
		{
			...
			execute = mysql_query(sqlcon, sprintf("SELECT `model`, `state`, `insurance` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d';", GetPlayerSQLID(playerid), VEHICLE_TYPE_PLAYER));
		...
		case 9: // Sell
		{
			...
			if((vehicle_index = Vehicle_ReturnID(GetPlayerVehicleID(playerid))) != RETURN_INVALID_VEHICLE_ID)
			{
				if(Vehicle_GetType(vehicle_index) == VEHICLE_TYPE_PLAYER && GetPlayerSQLID(playerid) == Vehicle_GetExtraID(vehicle_index))
				{
			        ...
			        SendServerMessage(playerid, "Kamu menawarkan "CYAN"%s "WHITE"seharga "GREEN"%s "WHITE"kepada "YELLOW"%s.", GetVehicleNameByVehicle(GetPlayerVehicleID(playerid)), FormatMoney(price), ReturnName(targetid, 0));
			        SendServerMessage(targetid, ""YELLOW"%s "WHITE"menawarkan "CYAN"%s "WHITE"seharga "GREEN"%s "WHITE"'/approve car' untuk menerima.", ReturnName(playerid, 0), GetVehicleNameByVehicle(GetPlayerVehicleID(playerid)), FormatMoney(price));
				}
...
CMD:gpc(playerid, params[])
	return callcmd::gotoplayercars(playerid, params);
...
CMD:pcl(playerid, params[])
	return callcmd::playercarlist(playerid, params);
...
CMD:playercarlist(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `model`, `state`, `insurance` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d';", GetPlayerSQLID(targetid), VEHICLE_TYPE_PLAYER));
...
CMD:dva(playerid, params[])
	return callcmd::destroyvehall(playerid, "\0");
...
CMD:hpark(playerid, params[]) {
    return callcmd::housepark(playerid,"");
}
CMD:hunpark(playerid, params[]) {
    return callcmd::houseunpark(playerid,"");
}
CMD:houseunpark(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model` FROM `server_vehicles` WHERE `house_parking` ='%d' AND `extraid`='%d' AND `type`='%d';", house_index, GetPlayerSQLID(playerid), VEHICLE_TYPE_PLAYER));
...
Dialog:HouseUnparkVehicle(playerid, response, listitem, inputtext[]) 
{
	if(response)
	{
		...
		mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `house_parking`='-1',`state`='%d' WHERE `id`='%d';", VEHICLE_STATE_SPAWNED ,id));
...
Dialog:HouseParkVehicle(playerid, response, listitem, inputtext[]) 
{
	if(response)
	{
		...
		if(Vehicle_IsOwned(playerid, id))
		{
			...
			mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `house_parking`='%d',`state`='%d' WHERE `id`='%d';", house_index, VEHICLE_STATE_HOUSEPARKED, VehicleData[id][vehIndex]));
...
CMD:park(playerid, params[])
{
	...
	if(Vehicle_IsOwned(playerid, vehicle_index))
	{
		...
		mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `parking`='1',`state`='%d' WHERE `id`='%d';", VEHICLE_STATE_PARKED, VehicleData[vehicle_index][vehIndex]));
...
CMD:unpark(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model` FROM `server_vehicles` WHERE `parking` ='1' AND `extraid`='%d' AND `type`='%d';", GetPlayerSQLID(playerid), VEHICLE_TYPE_PLAYER));
...
Dialog:UnparkVehicle(playerid, response, listitem, inputtext[]) 
{
	if(response)
	{
		...
		{
			mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `parking`='0',`posX`='%.2f',`posY`='%.2f',`posZ`='%.2f',`posRZ`='%.2f',`state`='%d' WHERE `id`='%d';", parkingPosition[index][0], parkingPosition[index][1], parkingPosition[index][2], parkingPosition[index][3], VEHICLE_STATE_SPAWNED ,id));
		}

		// if(IsPlayerInRangeOfPoint(playerid, 5.0, 1224.6685,-1376.7126,14.9861))
		// {
		// 	mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `parking`='0',`posX`='%.2f',`posY`='%.2f',`posZ`='%.2f',`posRZ`='%.2f',`state`='%d' WHERE `id`='%d';", parkingPosition2[index2][0], parkingPosition2[index2][1], parkingPosition2[index2][2], parkingPosition2[index2][3], VEHICLE_STATE_SPAWNED ,id));
		// }
...
CMD:rfc(playerid, params[])
	return callcmd::respawnfactioncars(playerid, "\1");
...
CMD:buyinsurance(playerid, params[])
{
	...
	{
		GetVehiclePos(VehicleData[vehicle][vehVehicleID], x, y, z);
		strcat(vehicle_list, sprintf(WHITE"%s\t"YELLOW"%d\t"GREEN"%s\n", GetVehicleNameByVehicle(VehicleData[vehicle][vehVehicleID]), VehicleData[vehicle][vehInsurance], FormatMoney(Vehicle_InsurancePrice(Model_GetCategory(VehicleData[vehicle][vehModel])))));
		g_selected_vehicle[playerid][count++] = vehicle;
	}
...
CMD:claiminsurance(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model`, `insurancetime` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(playerid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_INSURANCE));
...
CMD:forceinsurance(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model`, `insurancetime` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(targetid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_INSURANCE));
...
CMD:forcespawn(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(targetid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_HOUSEPARKED));
...
CMD:forcedeath(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT `id`, `model`, `insurancetime` FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(targetid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_DEAD));
...
CMD:impound(playerid, params[])
{
    ...
    if((vehicleid = Vehicle_ReturnID(GetVehicleTrailer(GetPlayerVehicleID(playerid)))) != RETURN_INVALID_VEHICLE_ID)
    {
        ...
        {
            SendServerMessage(i, "Your "YELLOW"%s "WHITE"has been impounded by {2641FE}%s "WHITE"for "COL_GREEN"%s.", GetVehicleNameByModel(VehicleData[vehicleid][vehModel]), FactionData[PlayerData[playerid][pFaction]][factionName], FormatMoney(price));
        ...
        mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET impoundtime=%d, state=%d, impoundprice=%d WHERE id=%d;", unimpound_date, VEHICLE_STATE_IMPOUND, price, VehicleData[vehicleid][vehIndex]));
		mysql_tquery(sqlcon, sprintf("UPDATE server_vehicles SET `posX`='%.2f',`posY`='%.2f',`posZ`='%.2f',`posRZ`='%.2f' WHERE `id`='%d';", ImpoundData[impound_id][impoundRelease][0], ImpoundData[impound_id][impoundRelease][1], ImpoundData[impound_id][impoundRelease][2], ImpoundData[impound_id][impoundRelease][3], VehicleData[vehicleid][vehIndex]));

        DetachTrailerFromVehicle(GetPlayerVehicleID(playerid));
        SendFactionMessage(PlayerData[playerid][pFaction], COLOR_RADIO, "RADIO: %s has impounded a %s for %s.", ReturnName(playerid, 0), GetVehicleNameByModel(VehicleData[vehicleid][vehModel]), FormatMoney(price));
...
CMD:releasecar(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT * FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(playerid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_IMPOUND));
	...
	if(cache_num_rows())
	{
		...
		for(new i = 0; i != cache_num_rows(); i++)
		{
			...
			strcat(output, sprintf(WHITE"%s\t%s\t%s\n", GetVehicleNameByModel(model), FormatMoney(fined), ConvertTimestamp(Timestamp:time)));
		}
...
CMD:forcerelease(playerid, params[])
{
	...
	execute = mysql_query(sqlcon, sprintf("SELECT * FROM `server_vehicles` WHERE `extraid`='%d' AND `type`='%d' AND `state`='%d';", GetPlayerSQLID(targetid), VEHICLE_TYPE_PLAYER, VEHICLE_STATE_IMPOUND));
	...
	if(cache_num_rows())
	{
		...
		for(new i = 0; i != cache_num_rows(); i++)
		{
			...
			g_selected_vehicle[playerid][i] = id;
			strcat(output, sprintf(WHITE"%s\t%s\t%s\n", GetVehicleNameByModel(model), FormatMoney(price), ConvertTimestamp(Timestamp:time)));
		}
...
CMD:refuel(playerid, params[])
{
    ...
    if((index = Pump_Nearest(playerid)) != -1)
    {
        ...
        if(GetMoney(playerid) < BusinessData[PumpData[index][pumpBusiness]][bizPrices][5]) 
        	return SendErrorMessage(playerid, "Minimal kamu harus memiliki "GREEN"%s "WHITE"untuk mengisi bahan bakar.", FormatMoney(BusinessData[PumpData[index][pumpBusiness]][bizPrices][5]));
...
CMD:icp(playerid, params[]) 
    return callcmd::installcustomplate(playerid, params);
...
CMD:installcustomplate(playerid, params[])
{
    ...
    if((id = Vehicle_ReturnID(vehicleid)) != RETURN_INVALID_VEHICLE_ID)
    {
		...
		mysql_tquery(sqlcon, sprintf("UPDATE `server_vehicles` SET `plate`='%s' WHERE `id`='%d';", VehicleData[id][vehPlate], VehicleData[id][vehIndex]));
...
CMD:registerplate(playerid, params[])
{
	...
	if((vehicle_index = Vehicle_ReturnID(vehicleid)) != RETURN_INVALID_VEHICLE_ID)
	{
		...
		mysql_tquery(sqlcon, sprintf("UPDATE `server_vehicles` SET `plate`='%s' WHERE `id`='%d';", VehicleData[vehicle_index][vehPlate], VehicleData[vehicle_index][vehIndex]));
